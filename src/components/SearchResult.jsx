...
function SearchResult() {
  ...
  useEffect(() => {
    if (publications.length > 0 && publications[0].ok.content.markup) {
      ...
      const text = xmlDoc.getElementsByTagName("sentence");
      const textSecond = xmlDocSecond.getElementsByTagName("sentence");

      if (text.length > 0 && text) {
        let str = "";
        for (let key in text) {
          str += text[key]["textContent"];
        }
        setXml(str.replace(/<[^>]*>/g, "").slice(0, 1200) + '...');
      }

      if (textSecond.length > 0 && textSecond) {
        let str = "";
        for (let key in textSecond) {
          str += textSecond[key]["textContent"];
        }
        setXmlSecond(str.replace(/<[^>]*>/g, "").slice(0, 1200) + '...');
      }
    }
